...
def test(args, split, loader, models, log, epoch, recorder):
    ...
    with torch.no_grad():
        for i, sample in enumerate(loader, start=1):
            ...
            recorder, iter_res, error = prepareRes(args, data, pred_c, pred, recorder, log, split)
            res.append(iter_res)
        
            should_display = (i % disp_intv == 0)
            should_save = (i % save_intv == 0)
            should_stop = (stop_iters > 0 and i >= stop_iters)
        
            if should_display:
                log.printItersSummary({
                    'split': split,
                    'epoch': epoch,
                    'iters': i,
                    'batch': len(loader),
            ...
            if should_save:
                results, nrow = prepareSave(args, data, pred_c, pred)
                log.saveImgResults(results, split, epoch, i, nrow=nrow, error='')
                log.plotCurves(recorder, split, epoch=epoch, intv=disp_intv)
            if should_stop:
                break
